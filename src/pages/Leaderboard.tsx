...
export default function Leaderboard() {
      ...
      try {
        ...
        if (firebaseUsers.length > 0) {
          ...
          defaultUsers.forEach(du => {
            if (!merged.some(mu => mu.displayName === du.displayName)) {
              merged.push(du);
            }
          });
          setUsersList(merged);
  ...
  // Security gating: Basic users can ONLY access 'school' leaderboard
      ...
      <div className="max-w-4xl mx-auto">
        ...
        <div className="p-6 bg-white rounded-3xl border border-slate-100 shadow-md flex flex-wrap items-center justify-between gap-4 mb-8">
          <div className="flex items-center gap-3">
            <div className={`p-3 rounded-2xl ${profile.plan !== 'basic' ? 'bg-purple-50 text-purple-700' : 'bg-slate-50 text-slate-500'}`}>
              <Sparkles size={20} />
            </div>
            <div>
              ...
              <h4 className="text-sm font-black text-slate-800 uppercase">
                {profile.plan === 'trial' ? '24h Сул Туршилт' : profile.plan === 'basic' ? 'Basic (Үнэгүй)' : profile.plan === 'pro' ? 'Pro лиценз' : 'Premium VIP'}
              </h4>
            </div>
          </div>
