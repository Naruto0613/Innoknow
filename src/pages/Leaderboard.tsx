...
export default function Leaderboard() {
  ...
  useEffect(() => {
    const fetchLeaderboard = async () => {
      ...
      const defaultUsers: BoardUser[] = [
        ...
        {
          uid: "u9",
          displayName: "Наранбаатар Жаргал",
          school: "ШУТИС",
          level: "C1",
          xp: 1710,
          streak: 29,
      ...
      try {
        ...
        Object.values(localProfiles).forEach((p: any) => {
          if (p && p.displayName) {
            const exIdx = defaultUsers.findIndex(
              (u) =>
                u.uid === p.uid ||
      ...
      if (profile) {
        const filteredDefaults = defaultUsers.filter(
          (u) => u.uid !== profile.uid && u.displayName !== profile.displayName,
        );
      ...
      try {
          ...
          defaultUsers.forEach((du) => {
            if (
              !merged.some(
                (mu) =>
                  mu.uid === du.uid ||
                  mu.displayName.toLowerCase() === du.displayName.toLowerCase(),
              )
            ) {
              merged.push(du);
            }
        ...
        console.warn(
          "Could not load users collection from Firestore direct query. Loading local list:",
          err,
        );
        setUsersList(defaultUsers);
      } finally {
  ...
  const filterUsers = () => {
    ...
    if (activeTab === "school") {
      ...
      list = list.filter(
        (u) => u.school.toLowerCase() === userSchool.toLowerCase(),
      );
  ...
  const getRankBadge = (rank?: number) => {
    ...
    return (
      <span className="text-slate-400 font-extrabold font-mono text-sm">
        #{rank}
      </span>
    );
  };
    ...
    <div className="min-h-[calc(100vh-64px)] bg-[#FDFCFB] py-12 px-4 md:px-8">
      <div className="max-w-4xl mx-auto">
        ...
        <div className="text-center mb-10">
          ...
          >
            <Trophy size={14} className="text-amber-500 animate-spin" /> АНГЛИ
            ХЭЛНИЙ ЧАРТЛИСТ
          </motion.div>
          ...
          <p className="text-slate-500 mt-2 font-medium text-xs md:text-sm">
            Нийт сурагчидтай өөрийн цуглуулсан XP оноогоор өрсөлдөж, ур чадвараа
            баталгаажуул.
          </p>
        </div>
          ...
          <div className="flex gap-6">
            <div className="text-center sm:text-right">
              ...
              <h4 className="text-sm font-bold text-slate-800">
                {profile.school || "Сургууль байхгүй"}
              </h4>
            </div>
            <div className="text-center sm:text-right">
              <p className="text-[10px] font-black tracking-widest uppercase text-slate-400">
              ...
              <h4 className="text-sm font-black text-[#58007E]">
                {profile.xp || 0} XP
              </h4>
            </div>
          </div>
        ...
        <div className="flex border-b border-slate-100 mb-6 overflow-x-auto gap-2">
          {[
            {
              id: "school" as const,
              label: "Сургуулийн чарт",
              icon: Users,
              isLocked: false,
            },
            {
              id: "national" as const,
              label: "Улсын хэмжээний чарт",
              icon: MapPin,
              isLocked: isBasic,
            },
            {
            ...
            return (
              ...
              >
                ...
                {tab.isLocked && (
                  <Lock
                    size={12}
                    className="text-red-500 ml-1 shrink-0 animate-pulse"
                  />
                )}
              </button>
            );
        ...
        <AnimatePresence mode="wait">
          {!hasAccess ? (
            ...
            >
              ...
              <h3 className="text-xl font-black font-serif italic text-slate-900 mb-3">
                Хязгаарлагдсан Цэс
              </h3>
              <p className="text-sm font-semibold text-slate-500 max-w-md mx-auto leading-relaxed mb-8">
                Улсын чартлист болон Өдөр тутмын тэргүүлэгчдийг үзэхийн тулд{" "}
                <span className="text-[#58007E]">Pro</span> эсвэл{" "}
                <span className="text-[#58007E]">Premium</span> гишүүнчлэлд
                нэгдсэн байх шаардлагатай.
              </p>
              <a
                href="/pricing"
                ...
                {loading ? (
                  <div className="text-center py-12">
                    <p className="text-xs font-black text-slate-400 uppercase tracking-widest animate-pulse">
                      Тэргүүлэгчдийг ачаалж байна...
                    </p>
                  </div>
                ) : sortedLeaderboard.length === 0 ? (
                  <div className="text-center py-12">
                    <p className="text-xs font-bold text-slate-400 uppercase">
                      Хараахан сурагч бүртгэгдээгүй байна.
                    </p>
                  </div>
                ) : (
                      ...
                      return (
                        <div
                          ...
                          className={`flex items-center justify-between p-4 px-6 md:px-8 transition-all ${
                            isMe
                              ? "bg-purple-50/50 border-l-4 border-l-[#58007E]"
                        ...
                        >
                          <div className="flex items-center gap-4">
                            ...
                            <div>
                              <div className="flex items-center gap-1.5">
                                <h4 className="text-xs md:text-sm font-black text-slate-800">
                                  {user.displayName}
                                </h4>
                                {hasPaidPlan && (
                          ...
                          <div className="flex items-center gap-6">
                            ...
                            <div className="text-right">
                              <span className="text-sm font-black text-[#58007E] font-sans">
                                {user.xp}
                              </span>
