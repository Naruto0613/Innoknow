...
export default function AdminPanel() {
  ...
  const [activeSubTab, setActiveSubTab] = useState<'pending' | 'active' | 'trials' | 'stats'>('pending');
  ...
  // Fetch requests & profiles database from firebase and falling back to localstorage
  const loadData = async () => {
    try {
      ...
      if (fbReqs.length > 0 || fbUsers.length > 0) {
        setRequests(fbReqs);
        setProfilesList(fbUsers);
        return;
      }
    } catch (e) {
      console.warn("Could not query firestore, using local registry:", e);
    }
    ...
    const legacyUsersList = Object.values(allProfiles).map((u: any) => ({
    ...
    setProfilesList(legacyUsersList);
  };
  ...
  const handleExtendAccess = async (userId: string, email: string, days: number) => {
    ...
    try {
      ...
      if (uSnap.exists()) {
        ...
        await updateDoc(uRef, {
          plan: uData.plan === 'basic' ? 'pro' : uData.plan, // upgrade label
          paymentStatus: 'Approved',
                          ...
                          {activePaidUsers.map((userProf: any) => {
                            ...
                            return (
                              <tr key={userProf.email} className="hover:bg-zinc-50/50 transition-colors">
                                <td className="p-6">
                                  <p className="font-extrabold text-sm text-zinc-905">{userProf.displayName}</p>
                      ...
                      <table className="w-full text-left">
                        ...
                        <tbody className="divide-y divide-zinc-100 font-medium text-xs">
                          {activeTrialUsers.map((p: any) => {
                            const endMs = new Date(p.trialEnd).getTime();
                            const remainMs = endMs - currentTime;
                            let tickerText = 'Ended';
                            
                            if (remainMs > 0) {
                              const hr = Math.floor(remainMs / (1000 * 60 * 60)).toString().padStart(2, '0');
