...
export default function AdminPanel() {
  ...
  const [activeSubTab, setActiveSubTab] = useState<
    "pending" | "active" | "trials" | "stats" | "feedback"
  >("pending");
  ...
  const loadData = async () => {
    ...
    try {
      ...
      const loadedRequests = JSON.parse(
        localStorage.getItem("innoknow_joining_requests") || "[]",
      );
      const premiumReqsMap = JSON.parse(
        localStorage.getItem("innoknow_payment_requests") || "{}",
      );
      const premiumReqsList = Object.entries(premiumReqsMap).map(
        ([uid, r]: [string, any]) => ({
          id: uid,
          userId: uid,
          ...r,
      ...
      loadedRequests.forEach((r: any) => {
        if (r) mergedMap.set(r.id || r.userId, r);
      });
      premiumReqsList.forEach((r: any) => {
        if (r) mergedMap.set(r.id || r.userId, r);
      });
    ...
    try {
      ...
      const allProfiles = JSON.parse(
        localStorage.getItem("innoknow_user_profiles") || "{}",
      );
    ...
    try {
      ...
      fbFeedbacks = JSON.parse(
        localStorage.getItem("innoknow_feedbacks") || "[]",
      );
    }

    // Merge localized storage additions to be safe
    const localFbs = JSON.parse(
      localStorage.getItem("innoknow_feedbacks") || "[]",
    );
  ...
  const handleChangeFeedbackStatus = async (
    feedbackId: string,
    ...
    const updated = loaded.map((f: any) =>
  ...
  const handleAcceptRequest = async (requestId: string) => {
    try {
      ...
      if (reqSnap.exists()) {
        ...
        alert(
          `🎉 Хэрэглэгчийн ${planToActivate.toUpperCase()} эрх амжилттай идэвхжлээ!`,
        );
        loadData();
        return;
      }
    ...
    let targetReq = loadedRequests.find(
      (req: any) => req.id === requestId || req.userId === requestId,
    );
    ...
    if (targetReq) {
      ...
      userProfile.accessExpiry = new Date(
        Date.now() + 30 * 24 * 60 * 60 * 1000,
      ).toISOString();
      allProfiles[targetEmail] = userProfile;

      // Clean up requests
      const filteredJoining = loadedRequests.filter(
        (req: any) => req.id !== requestId && req.userId !== requestId,
      );
      ...
      localStorage.setItem(
        ...
        JSON.stringify(filteredJoining),
      );
      localStorage.setItem(
        "innoknow_payment_requests",
        JSON.stringify(premiumReqsMap),
      );
      localStorage.setItem(
        "innoknow_user_profiles",
        JSON.stringify(allProfiles),
      );

      alert(
        `🎉 Хэрэглэгч ${targetReq.userName || targetReq.name || "Сурагч"}-д 30 хоногийн ${planToActivate.toUpperCase()} сургалтын эрх идэвхжүүллээ.`,
      );
    }
  ...
  const handleRejectRequest = async (requestId: string) => {
    try {
      ...
      if (reqSnap.exists()) {
        ...
        await deleteDoc(reqRef);
        console.log(
          `[MAIL SERVER] Triggered rejection email notification template to: ${email}`,
        );
  ...
  const handleExtendAccess = async (
    ...
    try {
      ...
      if (uSnap.exists()) {
        const uData = uSnap.data() || {};
        const currentExp = uData.accessExpiry
          ? new Date(uData.accessExpiry).getTime()
          : Date.now();
    ...
    if (allProfiles[email]) {
      ...
      const start = prof.accessExpiry
        ? new Date(prof.accessExpiry).getTime()
        : Date.now();
      prof.isPaid = true;
  ...
  const activePaidUsers = useMemo(() => {
    return profilesList.filter(
      (p) =>
        ...
        p.accessExpiry &&
        new Date(p.accessExpiry).getTime() > Date.now(),
    );
  }, [profilesList]);
  ...
  const activeTrialUsers = useMemo(() => {
    return profilesList.filter(
      (p) =>
        p.plan === "trial" &&
        p.trialEnd &&
        new Date(p.trialEnd).getTime() > currentTime,
    );
  ...
  const otherAdminEmail =
    user.uid === ADMIN_UIDS[0] ? "admin2@innoknow.mn" : "admin1@innoknow.mn";

  return (
    <div className="min-h-screen bg-[#FDFCFB] py-12 px-6">
      <div className="max-w-6xl mx-auto space-y-8">
        {/* Admin Header Context Banner */}
        <header className="bg-slate-900 text-white rounded-5xl p-8 md:p-10 flex flex-col md:flex-row items-center justify-between shadow-2xl relative overflow-hidden gap-6">
          ...
          <div className="z-10 bg-white/5 border border-white/10 px-5 py-4 rounded-2xl text-xs space-y-1.5 text-left max-w-sm">
            <p className="font-bold text-emerald-300 uppercase tracking-widest text-[9px]">
              💡 СУРГАЛТЫН ЭРХ ИДЭВХЖҮҮЛЭХ ЗААВАР
            </p>
            <p className="text-[11px] text-zinc-300 font-medium leading-relaxed">
              Хэрэглэгчийн хүсэлтийг зөвшөөрөхөд ердөө{" "}
              <span className="text-emerald-300 font-black">
                1 админы баталгаажуулалт
              </span>{" "}
              хангалттай бөгөөд эрх нь шууд хязгааргүй суралцахаар идэвхжинэ.
            </p>
          </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          <div className="bg-white p-6 rounded-4xl border border-zinc-100 shadow-sm text-center">
            <p className="text-[10px] font-black uppercase text-zinc-400 tracking-[0.2em] mb-1">
              Total Registered
            </p>
            <p className="text-3xl font-black text-zinc-900">
              {parsedStats.totalUsers} users
            </p>
          </div>
          <div className="bg-white p-6 rounded-4xl border border-zinc-100 shadow-sm text-center">
            <p className="text-[10px] font-black uppercase text-zinc-400 tracking-[0.2em] mb-1">
          ...
          <div className="bg-white p-6 rounded-4xl border border-zinc-100 shadow-sm text-center">
            ...
            <p className="text-3xl font-black text-amber-500">
              {parsedStats.totalActiveTrial} trials
            </p>
          </div>
        </div>
        ...
        <div className="space-y-6">
          <AnimatePresence mode="wait">
            {activeSubTab === "pending" && (
              ...
              >
                {requests.length === 0 ? (
                  <div className="p-20 text-center bg-white border border-zinc-100 rounded-5xl shadow-sm">
                  ...
                  <div className="grid md:grid-cols-2 gap-6">
                    {requests.map((req: any) => {
                      ...
                      return (
                        <div
                          key={req.id}
                          className="bg-white p-8 rounded-5xl border border-zinc-100 shadow-sm relative overflow-hidden flex flex-col justify-between min-h-90"
                        >
                          <div>
                            ...
                            <div className="bg-purple-50 border border-purple-100 rounded-2xl p-4 my-3 text-purple-950 font-bold text-xs flex flex-col gap-1.5 text-left">
                              <span className="text-[9px] uppercase tracking-wider text-[#58007E]/70 font-black">
                                Лавлах код (Гүйлгээний утга):
                              </span>
                              <div className="flex flex-wrap items-center gap-1.5 text-sm">
                                ...
                                <span className="text-xs text-purple-700 font-medium">
                                  requested{" "}
                                  <strong className="text-[#58007E] font-black">
                                    {(req.plan || "pro").toUpperCase()}
                                  </strong>{" "}
                                  version
                                </span>
                              </div>
                            ...
                            <div className="border-t border-b border-dashed border-zinc-100 py-4 my-4 space-y-2 text-left">
                              <p className="text-xs text-zinc-500 font-medium">
                                <strong>Хүсэлт гаргасан огноо:</strong>{" "}
                                {req.requestedAt || req.submittedAt
                                  ? new Date(
                                      req.requestedAt || req.submittedAt,
                                    ).toLocaleDateString()
                                  : "N/A"}
                              ...
                              <p className="text-xs text-zinc-500 font-medium">
                                <strong>Төхөөрөмжийн ID:</strong>{" "}
                                <span className="font-mono text-zinc-805 bg-gray-50 px-1.5 py-0.5 rounded text-[11px]">
                                  {req.deviceId || "N/A"}
                                </span>
                              </p>
                            </div>
                            ...
                            <div className="text-[10px] font-bold text-emerald-600 bg-emerald-50 border border-emerald-100 rounded-xl px-3 py-2 flex items-center gap-2 mb-6 text-left">
                              ...
                              <span>
                                Ердөө 1 админ зөвшөөрөл шаардлагатай (Single
                      ...
                      <table className="w-full text-left">
                        <thead>
                          <tr className="bg-zinc-50/50 border-b border-zinc-100">
                            <th className="p-6 text-[10px] font-black uppercase text-zinc-400 tracking-wider">
                              User Details
                            </th>
                            <th className="p-6 text-[10px] font-black uppercase text-zinc-400 tracking-wider">
                              Level & XP
                            ...
                            <th className="p-6 text-[10px] font-black uppercase text-zinc-400 tracking-wider text-right">
                              Actions
                            </th>
                          </tr>
                        </thead>
                        <tbody className="divide-y divide-zinc-100 font-medium text-xs">
                          {activePaidUsers.map((userProf: any) => {
                            const daysRemain = Math.ceil(
                              (new Date(userProf.accessExpiry).getTime() -
                                currentTime) /
                                (1000 * 60 * 60 * 24),
                            );
                            const dateFormatted = new Date(
                              userProf.accessExpiry,
                            ).toLocaleDateString();
                              ...
                              >
                                <td className="p-6">
                                  <div className="flex items-center gap-2">
                                    ...
                                    <span
                                      className={`text-[9px] px-2 py-0.5 rounded-full font-black uppercase tracking-widest ${userProf.plan === "premium" ? "bg-amber-100 text-amber-700 border border-amber-200" : "bg-blue-100 text-blue-700 border border-blue-200"}`}
                                    >
                                      {userProf.plan}
                                    </span>
                                  </div>
                                ...
                                <td className="p-6">
                                  <span className="bg-[#58007E]/5 text-[#58007E] px-2.5 py-1 rounded-md font-black mr-2 uppercase">
                                    {userProf.level || "A2"}
                                  </span>
                                ...
                                <td className="p-6 text-amber-600 font-bold flex items-center gap-1.5 pt-8">
                                  <Flame size={14} /> {userProf.streak || 0}{" "}
                                  days
                                </td>
                                <td className="p-6">
                          ...
                          {activeTrialUsers.map((p: any) => {
                            ...
                            return (
                              <tr
                                ...
                                className="hover:bg-zinc-50/50 transition-colors"
                              >
                                <td className="p-6 font-extrabold text-[#58007E]">
                                  {p.email}
                                </td>
                                <td className="p-6 font-mono text-[11px] text-zinc-500">
                                  {p.deviceId || "N/A"}
                                </td>
                                <td className="p-6 text-zinc-400 font-bold">
                                  {p.trialStart
                                    ? new Date(p.trialStart).toLocaleString()
                                    : "N/A"}
                                </td>
            ...
            {activeSubTab === "stats" && (
              ...
              >
                {/* Secondary stats 1 */}
                <div className="bg-white p-8 rounded-5xl border border-zinc-100 shadow-sm flex flex-col justify-between min-h-55">
                  <div>
                    <h4 className="text-[10px] font-black uppercase text-zinc-400 tracking-wider mb-2">
                      Platform Engagement
                    </h4>
                    <p className="text-xs text-zinc-500 font-semibold leading-relaxed">
                      The mathematical mean points (XP) of students registered
                      on INNOKNOW Prep:
                    </p>
                  </div>
                  <div>
                    <p className="text-4xl font-black text-[#58007E] font-serif italic mt-4">
                      {parsedStats.avgXp} XP
                    </p>
                    <p className="text-[9px] font-bold text-zinc-400 uppercase tracking-widest mt-1">
                      Average XP per account
                    </p>
                  </div>
                </div>
                ...
                <div className="bg-white p-8 rounded-[40px] border border-zinc-100 shadow-sm space-y-4">
                  ...
                  <div className="space-y-3">
                    {["A1", "A2", "B1", "B2", "C1", "C2"].map((lvl) => {
                      ...
                      const percentage = parsedStats.totalUsers
                        ? Math.round((count / parsedStats.totalUsers) * 100)
                        : 0;
                      ...
                      return (
                        ...
                        >
                          ...
                          <div className="flex-1 mx-4 h-2 bg-slate-50 border border-slate-100 rounded-full overflow-hidden">
                            <div
                              className="bg-[#58007E] h-full rounded-full"
                              style={{ width: `${percentage}%` }}
                            />
                          </div>
                          <span className="w-16 text-right text-zinc-400">
                            {count} profiles ({percentage}%)
                          </span>
                        </div>
                      );
                    })}
            ...
            {activeSubTab === "feedback" && (
              ...
              >
                {/* Filtration bar */}
                <div className="bg-white p-6 rounded-4xl border border-zinc-100 shadow-sm flex flex-col md:flex-row gap-4 items-center justify-between">
                  <div className="flex items-center gap-2">
                    <Filter className="w-5 h-5 text-[#58007E]" />
                    <h3 className="font-bold text-sm text-zinc-800">
                      Санал хүсэлтийн шүүлтүүр
                    </h3>
                  </div>
                  ...
                  <div className="grid gap-6 md:grid-cols-2">
                    {filteredFeedbacks.map((f: any) => {
                      return (
                        <div
                          key={f.id}
                          className={`bg-white p-6 rounded-4xl border transition-all shadow-sm flex flex-col justify-between ${
                            f.status === "new"
                              ? "border-[#58007E]/30 ring-2 ring-[#58007E]/5"
                              : "border-zinc-100"
                          }`}
                        >
                          <div>
                            ...
                            <div className="flex justify-between items-start gap-4 mb-4">
                              <div>
                                <h4 className="font-extrabold text-[#58007E] text-base leading-tight">
                                  {f.userName}
                                </h4>
                                <p className="text-xs text-zinc-500 font-medium">
                                  {f.userEmail}
                          ...
                          <div className="flex items-center justify-end gap-2 mt-4 pt-3 border-t border-zinc-50 text-xs">
                            <span className="text-zinc-400 font-black text-[9px] uppercase tracking-wider mr-auto">
                              Төлөв өөрчлөх:
                            </span>
                            {f.status !== "read" && (
                              <button
                                onClick={() =>
                                  handleChangeFeedbackStatus(f.id, "read")
                                }
                            ...
                            {f.status !== "resolved" && (
                              <button
                                onClick={() =>
                                  handleChangeFeedbackStatus(f.id, "resolved")
                                }
                            ...
                            {f.status !== "new" && (
                              <button
                                onClick={() =>
                                  handleChangeFeedbackStatus(f.id, "new")
                                }
