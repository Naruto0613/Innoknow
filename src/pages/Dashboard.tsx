...
import { useAuth, ADMIN_UIDS, parseDate, isUserAdmin as checkIsAdmin } from '../hooks/useAuth';
...
export default function Dashboard() {
  ...
  const isTrialActive = profile.plan === 'trial' && !!(expiry && parseDate(expiry).getTime() > Date.now());
  ...
  if (isUserAdmin) {
    ...
    const expiryDate = profile.accessExpiry || profile.planExpiry ? parseDate(profile.accessExpiry || profile.planExpiry).getTime() : Date.now() + 30 * 24 * 60 * 60 * 1000;
  ...
  const isUnder24HoursLeft = isTrialActive && (expiryTime > 0) && (expiryTime - Date.now() <= 24 * 60 * 60 * 1000);
    ...
    <div className="max-w-7xl mx-auto p-4 md:p-8 flex flex-col gap-6 relative">
      ...
      {isUnder24HoursLeft && (
        <div className="bg-amber-50 border border-amber-200 rounded-3xl p-6 flex items-start gap-4 text-left shadow-sm">
          ...
          <div>
            <h4 className="text-sm font-black text-amber-950 uppercase tracking-tight">Хугацаа дуусах мэдэгдэл</h4>
            <p className="text-xs text-amber-800 font-bold mt-1">Таны trial дуусахад 1 өдөр үлдлээ</p>
            <p className="text-[11px] text-amber-700/80 font-medium mt-0.5">Туршилтын хугацаа 3 өдөр (72 цаг) бөгөөд дууссаны дараа систем автоматаар хаагдах тул доорх сонголтоос багцаа идэвхжүүлнэ үү.</p>
          </div>
        </div>
      )}
