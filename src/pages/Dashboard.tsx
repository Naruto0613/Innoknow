...
export default function Dashboard() {
  ...
  useEffect(() => {
    const expiryDate = profile?.trialExpiry || profile?.trialEnd;
    if (profile?.plan !== "trial" || !expiryDate) return;
  ...
  }, [profile?.trialExpiry, profile?.trialEnd, profile?.plan, navigate]);
  ...
  useEffect(() => {
    ...
    if (profile.plan === "trial") {
      const expiryDate = profile.trialExpiry || profile.trialEnd || "";
      const isExpired = parseDate(expiryDate).getTime() <= Date.now();
      if (isExpired) {
        navigate("/pricing");
      }
    }
    ...
    if (user) {
      ...
      if (storedProgress) {
        ...
        const initialProgress = [
          ...
          {
            title: "Vocabulary Core Core Set",
            type: "Flashcards",
            score: "100%",
            date: "2 days ago",
          },
  ...
  const expiry = profile.trialExpiry || profile.trialEnd;
  const isTrialActive =
    profile.plan === "trial" &&
    !!(expiry && parseDate(expiry).getTime() > Date.now());
  const hasAccess = isUserAdmin || isSubscriptionPaid || isTrialActive;
  ...
  if (isUserAdmin) {
    ...
    const expiryDate =
      profile.accessExpiry || profile.planExpiry
        ? parseDate(profile.accessExpiry || profile.planExpiry).getTime()
  ...
  // Under 24 hours left warning banner check
  const expiryTime = expiry ? parseDate(expiry).getTime() : 0;
  const isUnder24HoursLeft =
    isTrialActive &&
    expiryTime > 0 &&
  ...
  const handleRequestAccess = async () => {
    if (!confirmedTransfer) {
      setErrorMessage(
        "Please complete the bank transfer and confirm by checking the box.",
      );
      return;
    }
    ...
    try {
      ...
      const currentRequests = JSON.parse(
        localStorage.getItem("innoknow_joining_requests") || "[]",
      );
      ...
      const filteredRequests = currentRequests.filter(
        (r: any) => r.id !== profile.uid,
      );
      filteredRequests.push(newRequest);

      localStorage.setItem(
        "innoknow_joining_requests",
  ...
  return (
    <div className="max-w-7xl mx-auto p-4 md:p-8 flex flex-col gap-6 relative">
      ...
      {!hasAccess && (
        <div className="absolute inset-x-4 md:inset-x-8 top-4 md:top-8 bottom-4 md:bottom-8 z-40 bg-zinc-950/70 backdrop-blur-md rounded-[48px] flex flex-col items-center justify-center text-center p-8 border border-white/10">
          <motion.div
            ...
            className="max-w-xl bg-white rounded-5xl p-10 md:p-12 shadow-2xl border border-zinc-100 flex flex-col items-center text-center"
          >
            <div className="w-20 h-20 bg-amber-50 text-amber-500 rounded-4xl flex items-center justify-center shadow-inner mb-6">
              <AlertCircle size={40} />
            </div>

            <h2 className="text-4xl font-black font-serif italic text-zinc-900 tracking-tight leading-none mb-4">
              Your trial has ended
            </h2>
            <p className="text-zinc-500 text-sm font-medium leading-relaxed mb-8">
              We hope you enjoyed exploring the INNOKNOW platform! Unlock
              1-Month of unlimited learning access to speaking simulators,
              listening evaluations, and grammar modules for just{" "}
              <strong className="text-zinc-900 font-extrabold">5,000 ₮</strong>.
            </p>
            ...
            {profile.requestAccessStatus === "pending" ? (
              <div className="bg-slate-50 border border-slate-100 px-8 py-6 rounded-3xl w-full text-center space-y-2 mb-6">
                <p className="text-xs font-black text-slate-800 uppercase tracking-wider flex items-center justify-center gap-2">
                  ...
                  Pending Approval (Reviewing Payment)
                </p>
                <p className="text-xs text-zinc-400 font-medium">
                  We are waiting for BOTH administrators to accept your
                  subscription request. This usually takes 5 to 30 minutes!
                </p>
              </div>
              ...
              <div className="space-y-6 w-full">
                <div className="bg-red-50 border border-red-100 px-6 py-4 rounded-3xl text-left">
                  ...
                  <p className="text-xs text-red-400 font-medium mt-1">
                    We couldn't verify your transfer of 5,000 ₮. Make sure to
                    complete the transfer and request access again.
                  </p>
                </div>
                <button
      ...
      {/* Header Bar */}
      <div className="bg-white border border-zinc-100 rounded-5xl p-8 flex flex-col md:flex-row items-center justify-between shadow-sm gap-6">
        <div className="flex items-center gap-6">
          ...
          <div>
            <h1 className="text-3xl font-black tracking-tight text-zinc-900 italic font-serif">
            ...
            <div className="flex flex-col sm:flex-row sm:items-center gap-2 mt-1.5">
              <span className="text-[10px] font-black text-zinc-400 uppercase tracking-widest flex items-center gap-2 align-middle">
                Current Level:{" "}
                <span className="text-[#58007E] bg-[#58007E]/5 px-2 py-0.5 rounded-lg font-extrabold">
                  {profile?.level || "A2"}
                </span>
              </span>
              <span className="hidden sm:inline text-zinc-200">•</span>
              <span
                className={`text-[10px] font-black uppercase tracking-widest rounded-lg px-2 py-0.5 inline-block ${isSubscriptionPaid ? "text-emerald-500 bg-emerald-500/5" : isTrialActive ? "text-indigo-500 bg-indigo-500/5" : "text-amber-500 bg-amber-500/5"}`}
              >
                {trialStateLabel}
              </span>
            </div>
        ...
        <div className="flex items-center gap-4">
          <div className="bg-amber-50 px-6 py-3 rounded-2xl items-center gap-2 border border-amber-100 flex shadow-sm">
            ...
            <span className="text-[10px] font-black text-amber-700 tracking-widest uppercase">
      ...
      {isUnder24HoursLeft && (
        <div className="bg-amber-50 border border-amber-200 rounded-3xl p-6 flex items-start gap-4 text-left shadow-sm">
          ...
          <div>
            <h4 className="text-sm font-black text-amber-950 uppercase tracking-tight">
              Хугацаа дуусах мэдэгдэл
            </h4>
            <p className="text-xs text-amber-800 font-bold mt-1">
              Таны trial дуусахад 1 өдөр үлдлээ
            </p>
            <p className="text-[11px] text-amber-700/80 font-medium mt-0.5">
              Туршилтын хугацаа 3 өдөр (72 цаг) бөгөөд дууссаны дараа систем
              автоматаар хаагдах тул доорх сонголтоос багцаа идэвхжүүлнэ үү.
            </p>
          </div>
        </div>
      )}
      ...
      {profile.plan === "trial" && isTrialActive && (
        <div className="bg-linear-to-r from-purple-50 to-indigo-50 border border-[#58007E]/10 rounded-3xl p-6 flex flex-col md:flex-row items-center justify-between gap-4 shadow-sm">
          <div className="flex items-center gap-4">
            ...
            <div className="text-left">
              <p className="text-xs font-black text-slate-900 uppercase tracking-tight">
                Үнэгүй Туршилтын Хугацаа
              </p>
              <p className="text-sm font-black text-[#58007E] mt-0.5">
                Таны trial: {timeRemaining || "хэмжиж байна..."} үлдлээ
              </p>
            </div>
          </div>
          <Link
      ...
      <main className="grid lg:grid-cols-12 gap-6 items-stretch">
        ...
        <div className="lg:col-span-7 flex flex-col gap-6">
          {/* Active Tutor Bento */}
          <div className="bg-indigo-900 rounded-5xl p-8 relative overflow-hidden flex flex-col justify-between text-white shadow-2xl flex-1 min-h-75">
            <div className="z-10">
              ...
              <h2 className="text-4xl font-extrabold mt-6 leading-tight italic font-serif text-white text-left">
                Practice Speaking <br />
                with AI Tutors
              </h2>
              <p className="text-indigo-100/70 mt-3 max-w-sm text-sm font-medium text-left">
                Get real-time feedback on your IELTS Speaking Part 2 topics.
              </p>
            </div>
            ...
            <div className="z-10 flex items-center gap-6 mt-8">
              <Link
                to="/ielts"
                className="px-8 py-3.5 bg-white text-indigo-900 font-bold rounded-2xl hover:scale-105 transition-transform shadow-xl shadow-indigo-950/20"
              >
                Start Session
              </Link>
              <div className="flex items-center gap-3">
                <div className="flex -space-x-3">
                  {[1, 2, 3].map((i) => (
                    <div
                      key={i}
                      className={`w-8 h-8 rounded-full border-2 border-indigo-900 bg-slate-${300 + i * 100}`}
                    ></div>
                  ))}
                </div>
                <span className="text-[10px] font-bold text-indigo-300 uppercase tracking-widest">
                  12 others practicing
                </span>
              </div>
            </div>
          ...
          <div className="bg-white border border-slate-200 rounded-4xl p-6 shadow-sm">
            <h3 className="text-sm font-bold text-slate-800 mb-6 uppercase tracking-widest text-left">
              Quick Level Access
            </h3>
            <div className="grid grid-cols-3 md:grid-cols-6 gap-3">
              {["A1", "A2", "B1", "B2", "C1", "C2"].map((l) => (
                ...
                >
                  ...
                  <div className="text-[9px] font-bold opacity-60 uppercase mt-1">
                    Level
                  </div>
                </button>
              ))}
        ...
        <div className="lg:col-span-5 grid grid-cols-2 grid-rows-3 gap-6">
          {/* Main Stat 1 */}
          <div className="bg-emerald-50 border border-emerald-100 rounded-4xl p-6 flex flex-col justify-between shadow-sm">
            <div className="w-10 h-10 bg-emerald-500 rounded-xl flex items-center justify-center text-white shadow-lg shadow-emerald-200">
              <TrendingUp size={20} />
            </div>
            <div>
              <div className="text-3xl font-black text-emerald-900 tracking-tighter italic text-left">
                84%
              </div>
              <div className="text-[10px] font-black text-emerald-600 uppercase tracking-widest mt-1 text-left">
                Fluency Rank
              </div>
            </div>
          </div>
          ...
          {/* Main Stat 2 */}
          <div className="bg-violet-50 border border-violet-100 rounded-4xl p-6 flex flex-col justify-between shadow-sm">
            ...
            <div>
              ...
              <div className="text-[10px] font-black text-violet-600 uppercase tracking-widest mt-1 text-left">
                Words Learned
              </div>
            </div>
          </div>
          ...
          {/* Wide Progress Bento */}
          <div className="col-span-2 bg-slate-900 rounded-5xl p-8 text-white flex flex-col justify-between shadow-2xl relative overflow-hidden group">
            <div className="relative z-10 flex justify-between items-start">
              <div>
                <h4 className="text-[10px] font-black uppercase text-slate-500 tracking-[0.2em] mb-1 text-left">
                  Mastery Progress
                </h4>
                <div className="text-3xl font-black italic font-serif text-left">
                  Level {profile?.level || "A1"}
                </div>
              </div>
              <span className="text-[10px] font-black italic opacity-40">
                TARGET: C1
              </span>
            </div>
          ...
          {/* Feedback Card Bento */}
          <div className="col-span-2 bg-white border border-slate-200 rounded-4xl p-6 shadow-sm flex flex-col justify-between text-left">
            ...
            <p className="text-sm font-medium italic text-slate-700 leading-relaxed text-left">
              "Great handle on complex sentence structures! Try incorporating
              more phrasal verbs to sound more natural."
            </p>
            <div className="flex gap-2 mt-4 overflow-x-auto pb-1 scrollbar-hide">
              {["Idioms", "Syntax", "Accent"].map((tag) => (
                <span
                  key={tag}
                  className="px-2 py-1 bg-slate-50 border border-slate-100 rounded-lg text-[9px] font-black text-indigo-600 uppercase tracking-tighter"
                >
                  {tag}
                </span>
              ))}
      ...
      {/* Course Strip Bento */}
      <div className="bg-white border border-slate-200 rounded-4xl p-5 flex flex-col sm:flex-row items-center gap-8 shadow-sm">
        <div className="flex items-center gap-4 border-b sm:border-b-0 sm:border-r border-slate-100 pb-4 sm:pb-0 sm:pr-8 shrink-0 w-full sm:w-auto">
          <div className="w-12 h-12 bg-indigo-100 rounded-2xl flex items-center justify-center text-indigo-600 text-xl shadow-indigo-100 font-bold">
            🎓
          </div>
          <div className="text-left">
        ...
        <div className="flex-1 flex flex-col sm:flex-row gap-4 overflow-hidden w-full">
          {[
            ...
            <Link
              key={c.code}
              to="/courses"
              className="flex-1 bg-slate-50/50 border border-slate-100 rounded-2xl p-3 px-5 flex items-center justify-between hover:bg-white hover:shadow-xl hover:shadow-indigo-50 transition-all cursor-pointer group"
            >
              <div className="flex items-center gap-4 truncate">
                <div
                  className={`w-9 h-9 ${c.color} rounded-xl flex items-center justify-center text-white text-[10px] font-black shadow-lg shadow-black/10`}
                >
                  {c.code}
                </div>
              ...
              <span className="text-[10px] font-black text-slate-300 ml-4">
                {c.progress}%
              </span>
            </Link>
          ))}
        ...
        <Link
          to="/courses"
          className="p-3 bg-slate-900 text-white rounded-2xl hover:scale-105 transition-transform shadow-lg shadow-black/10 hidden sm:block"
        >
        ...
        {showPaymentModal && (
          <div className="fixed inset-0 z-100 flex items-center justify-center p-4">
            ...
            <motion.div
              ...
              className="bg-white w-full max-w-xl rounded-5xl shadow-2xl relative z-10 overflow-hidden flex flex-col"
            >
                ...
                <div className="relative z-10 flex items-center gap-3">
                  ...
                  <div>
                    <h3 className="text-2xl font-black font-serif italic">
                      Tuition Access Request
                    </h3>
                    <p className="text-[9px] font-black uppercase text-purple-200 tracking-wider">
                      Complete local bank wire transfer
                    </p>
                  </div>
                </div>
                ...
                <div className="bg-slate-50 p-6 rounded-3xl border border-slate-100 space-y-4">
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      ...
                      <p className="text-lg font-black text-[#58007E]">
                        5,000 ₮ / Month
                      </p>
                    </div>
                    <div>
                  ...
                  <div className="border-t border-slate-200/65 pt-4 space-y-2.5">
                    <div className="flex justify-between items-center bg-white border border-slate-100 px-4 py-2.5 rounded-xl">
                      ...
                      <span className="text-[9px] font-extrabold text-zinc-300">
                        Default
                      </span>
                    </div>
                    ...
                    <div className="flex justify-between items-center bg-white border border-slate-100 px-4 py-2.5 rounded-xl">
                      <div>
                        ...
                        <p className="text-sm font-black text-[#58007E] font-mono">
                          5222 111 351
                        </p>
                      </div>
                      <span className="text-[8px] font-black bg-[#58007E]/5 text-[#58007E] px-2 py-1 rounded">
                        Copy
                      </span>
                    </div>
                    ...
                    <div className="flex justify-between items-center bg-white border border-slate-100 px-4 py-2.5 rounded-xl">
                      <div>
                        ...
                        <p className="text-sm font-black text-zinc-850 font-mono">
                          30 0002 00
                        </p>
                      </div>
                    ...
                    <div className="flex justify-between items-center bg-indigo-50 border border-indigo-100 px-4 py-2.5 rounded-xl">
                      <div>
                        <p className="text-[8px] font-black uppercase text-indigo-500">
                          Required Memo Reference
                        </p>
                        <p className="text-xs font-black text-indigo-900 font-mono">
                          {user.email}
                        </p>
                      </div>
                      <span className="text-[8px] font-black text-indigo-600 bg-white border border-indigo-100 px-2 py-0.5 rounded animate-pulse">
                        Required
                      </span>
                    </div>
                  </div>
                ...
                <div className="space-y-3">
                  <label className="flex items-start gap-3 cursor-pointer select-none">
                    ...
                    <span className="text-xs font-semibold text-zinc-600 leading-relaxed">
                      I confirm that I have wired exactly{" "}
                      <strong className="text-zinc-950">5,000 ₮</strong> to the
                      Khaan Bank account above and specified my e-mail address{" "}
                      <strong className="text-[#58007E]">{user.email}</strong>{" "}
                      as the transactions' transfer reference.
                    </span>
                  </label>
                </div>
