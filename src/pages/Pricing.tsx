...
import { Check, ArrowRight, Copy, CreditCard, Shield, Sparkles, CheckCircle2 } from 'lucide-react';
...
export default function Pricing() {
  ...
  const plans = [
    {
      ...
      priceNum: 0,
      priceLabel: 'FREE',
      description: 'Англи хэлний суурь мэдлэг олгох анхан шатны хэрэглээ',
      features: [
        '1 lesson per day (Өдөрт 1 хичээл)',
        'Basic vocabulary only (Зөвхөн анхан шатны үгс)',
        'Limited battle (7 хоногт 1 удаа батл)',
        'Leaderboard (school only - зөвхөн сургуулийн)',
        '7 day max streak tracking (7 хүртэлх хоногийн streak)',
      ],
      color: 'border-slate-200 bg-white text-slate-800',
      btnText: 'Үнэгүй ашиглах',
    ...
    },
    {
      ...
      color: 'border-[#58007E] bg-white ring-2 ring-[#58007E]/20',
      btnText: 'Pro-д шилжих',
      ctaClass: 'bg-[#58007E] hover:bg-[#40005C] text-white',
      badge: 'Хамгийн их сонгосон'
    },
    {
      id: 'premium' as const,
      name: 'Premium',
      ...
      btnText: 'Premium-д шилжих',
      ctaClass: 'bg-white hover:bg-slate-100 text-slate-900',
      badge: 'VIP Сонголт'
    }
  ];
  ...
  const handleSelectPlan = async (planType: 'basic' | 'pro' | 'premium') => {
    if (planType === 'basic') {
      // Basic is free, instantly downgrade or activate
      try {
        await updateProfile({
          plan: 'basic',
          paymentStatus: 'Free',
        ...
        alert('Баяр хүргэе! Та Basic төлөвлөгөөг идэвхжүүллээ.');
        navigate('/dashboard');
      } catch (e) {
        console.error("Option state update failed:", e);
      }
    } else {
          ...
          {!selectedPlan ? (
            ...
            >
              {plans.map((p) => (
                ...
                >
                  {p.badge && (
                    <span className="absolute top-4 right-6 text-[10px] font-black uppercase tracking-widest text-[#58007E] bg-purple-50 px-3 py-1 rounded-full border border-purple-100">
                      {p.badge}
                    </span>
                  )}
                  <div>
