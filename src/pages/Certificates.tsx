...
import { Award, Download, Linkedin, QrCode, Search, Share2, Shield, Sparkles, CheckCircle2 } from 'lucide-react';
...
export default function Certificates() {
  ...
  const certificateTypes: CertificateData[] = [
    {
      ...
      cvValue: 'Энэхүү гэрчилгээ нь таныг англи хэлний суурь өгүүлбэрүүдийг ойлгох, өдөр тутмын энгийн харилцааг хөтлөх чадвартай болохыг ажил олгогчдод гэрчилнэ.'
    },
    {
      id: 'intermediate',
      type: 'intermediate',
      ...
      cvValue: 'Дунд шатны сертификат нь таныг мэргэжлийн сэдвүүдийг ойлгох, чөлөөтэй илэрхийлэх, өөртөө итгэлтэйгээр гадны түншүүдтэй имэйл бичих чадварыг батална.'
    },
    {
      id: 'advanced',
      type: 'advanced',
      title: 'Advanced Certificate (C1-C2)',
      description: 'Англи хэлний ахисан дунд болон дээд шатны мэдлэгийн гэрчилгээ.',
      cefr: 'CEFR C1 / C2 Advanced Competency',
      reqs: 'C1 болон C2 түвшний бүх дасгал цуврал хичээлүүдийг дуусгасан байх.',
      cvValue: 'Дээд шатны сертификат нь таныг бичгээр болон аман хэлбэрээр цогц илэрхийлэл өгөх, IELTS-ийн өндөр онооны урьдчилсан мэдлэгийг бүрэн эзэмшсэнийг баталдаг.'
    },
    {
      id: 'streak100',
      type: 'streak100',
      title: '100-Day Streak Milestone Certificate',
      description: 'Нийт 100 хоног тасралтгүй суралцсан тууштай сэтгэлгээний баталгаа.',
      cefr: '100-Day Consistency Guard',
      reqs: 'Тасралтгүй 100 өдрийн streak-г амжилттай дуусгаж барьж чадсан байх',
      cvValue: 'Суралцах чин эрмэлзэлтэй, тууштай суралцах сахилга баттай тул хамгийн хариуцлагатай ажилтан, манлайлагч гэдгийг тань баталдаг хүчирхэг баримт бичиг.'
    }
  ];
  ...
  const isEligible = (cert: CertificateData) => {
    ...
    if (cert.id === 'intermediate') {
      return (profile.lessonsCompleted || 0) >= 20 && (profile.level === 'B1' || profile.level === 'B2' || profile.level === 'C1' || profile.level === 'C2');
    }
    if (cert.id === 'advanced') {
      return (profile.lessonsCompleted || 0) >= 30 && (profile.level === 'C1' || profile.level === 'C2');
    }
  ...
  const handleClaimCertificate = async (cert: CertificateData) => {
    ...
    const certUID = 'CERT_' + Math.random().toString(36).substring(2, 11).toUpperCase();
    
    // QR Code source if Premium
    const qrSource = profile.plan === 'premium' 
    ...
    <div className="min-h-[calc(100vh-64px)] bg-[#FDFCFB] py-12 px-4 md:px-8 print:bg-white print:py-0">
      <div className="max-w-4xl mx-auto print:max-w-none">
        ...
        <div className="text-center mb-12 print:hidden">
          ...
          >
            <Award size={14} className="text-[#58007E] h-4 w-4 animate-pulse" /> INNOKNOW CERTIFICATIONS
          </motion.div>
          ...
          <p className="text-slate-500 mt-2 font-medium text-xs md:text-sm max-w-xl mx-auto">
            Таны англи хэлний сургалтын үр дүн, хичээл зүтгэлийг албан ёсоор баталгаажуулах CEFR стандартын дижитал сертификат.
          </p>
        </div>
        ...
        <AnimatePresence mode="wait">
          {selectedCert ? (
            ...
            >
              ...
              <div id="diploma-canvas" className="bg-white border-[12px] border-slate-800 p-8 md:p-16 rounded-[4px] relative shadow-2xl tracking-normal text-slate-800 max-w-2xl mx-auto relative overflow-hidden print:border-none print:shadow-none">
                ...
                <div className="relative text-center space-y-6">
                  ...
                  <div className="flex justify-center flex-col items-center gap-2">
                    <div className="w-14 h-14 bg-[#58007E] rounded-xl flex items-center justify-center text-white font-black text-2xl italic">I</div>
                    <span className="font-serif italic text-sm tracking-widest font-bold">INNOKNOW ACADEMY</span>
                  </div>

                  <h2 className="text-amber-600 font-serif text-xs uppercase tracking-[0.25em] font-bold">CERTIFICATE OF ACHIEVMENT</h2>
                  
                  <p className="text-slate-400 font-serif italic text-xs">This credential certifies that:</p>
                  ...
                  <p className="text-xs text-slate-500 max-w-md mx-auto leading-relaxed">
                    has successfully satisfied all standardized academic requirements and maintained pristine learning consistencies to be awarded the title of
                  </p>
                  ...
                  <p className="text-[11px] text-slate-400 font-medium">
                    Issued on {new Date().toLocaleDateString('mn-MN')} | Unique ref_ID: <span className="font-mono text-[#58007E] font-bold">INNO_CERT_{profile.uid.substring(3).toUpperCase() || 'REF_X'}</span>
                  </p>
                  ...
                  <div className="grid grid-cols-2 gap-8 pt-10 border-t border-dashed border-slate-200 mt-8 max-w-lg mx-auto">
                    ...
                    <div className="text-center">
                      <p className="font-serif italic text-[#58007E] font-bold text-sm leading-none h-6">S.Setsen</p>
                      <div className="border-t border-slate-350 pt-1.5 text-[9px] font-black uppercase text-slate-400 tracking-wider">Гүйцэтгэх захирал, Setsen</div>
                    </div>
                    {/* Sig 2 */}
                    <div className="text-center">
                      <p className="font-serif italic text-[#58007E] font-bold text-sm leading-none h-6">N.Nomin</p>
                      <div className="border-t border-slate-350 pt-1.5 text-[9px] font-black uppercase text-slate-400 tracking-wider">Сургалтын албаны дарга, Nomin</div>
                    </div>
                  </div>
                  ...
                  {profile.plan === 'premium' ? (
                    ...
                    <div className="pt-4 text-center">
                      <span className="text-[9px] text-slate-400 font-bold uppercase tracking-wider block">🔒 QR баталгаажуулалт зөвхөн Premium-д нээлттэй</span>
                    </div>
                  )}
              ...
              <div className="bg-slate-50 p-6 md:p-8 rounded-3xl border border-slate-100 max-w-2xl mx-auto print:hidden">
                <h4 className="text-xs font-black uppercase text-slate-500 tracking-wider mb-2">CV / Ажил хайх хэсэгт ашиглах байдал (Нэмүү өртөг)</h4>
                <p className="text-xs font-bold text-slate-500 leading-relaxed mb-4">{selectedCert.cvValue}</p>
                <div className="p-4 bg-white rounded-2xl border border-slate-150 text-[10px] text-slate-400 italic">
                  <strong>Загвар CV текстийн оруулга:</strong> "Монголын шилдэг шинэлэг платформ болох INNOKNOW-ээс CEFR-ийн стандартын англи хэлний түвшний гэрчилгээг тууштай суралцаж амжилттай дуусган олж авсан."
                </div>
              </div>
                  ...
                  >
                    <div>
                      <div className="flex justify-between items-start mb-6">
                        <div className={`p-4 rounded-3xl ${claimed ? 'bg-purple-100 text-[#58007E]' : 'bg-slate-50 text-slate-500'}`}>
                          <Award size={24} />
                        </div>
                        {claimed ? (
                      ...
                      <h3 className="text-lg font-bold font-serif mb-2 text-slate-800">{cert.title}</h3>
                      <p className="text-xs font-bold text-[#58007E] font-mono mb-4">{cert.cefr}</p>
                      
                      <div className="space-y-4 pt-4 border-t border-slate-50 mb-8">
                        <div>
                          <span className="text-[9px] text-slate-400 font-bold block uppercase tracking-wider mb-1">Олгох шалгуур:</span>
                          <p className="text-[11px] font-semibold text-slate-500 leading-relaxed">{cert.reqs}</p>
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="flex gap-4">
                      {claimed ? (
                        ...
                        >
                          {claimingCertId === cert.id ? 'Шалгаж байна...' : 'Сертификатыг баталгаажуулах'}
                        </button>
                      ) : (
                        <button
                          disabled
