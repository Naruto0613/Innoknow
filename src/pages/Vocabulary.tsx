...
export default function Vocabulary() {
  ...
  const [localLearnedWords, setLocalLearnedWords] = useState<{ wordId: string; dateLearned: string }[]>(() => {
    try {
  ...
  const toggleLearned = async (word: VocabularyWord) => {
    ...
    if (isLearned) {
      updatedList = learnedWordsList.filter((item: any) => item.wordId !== word.id);
    } else {
      updatedList = [
        ...learnedWordsList, 
    ...
    if (user && profile) {
      ...
      localStorage.setItem('innoknow_guest_learned_words', JSON.stringify(updatedList));
  ...
  const mappedLearnedDetailed = useMemo(() => {
    const recordsMap = new Map(learnedWordsList.map((item: any) => [item.wordId, item.dateLearned]));
    return allWords
  ...
  }, [activeWordList, searchTerm, selectedLevel, selectedCategory, selectedPos]);
      ...
      <div className="max-w-6xl mx-auto">
        <AnimatePresence mode="wait">
          {!flashcardMode ? (
            ...
            >
              {/* Header section */}
              <header className="flex flex-col md:flex-row justify-between items-start md:items-end gap-6 bg-white border border-zinc-100 rounded-[40px] p-8 shadow-sm">
                <div>
                  <h1 className="text-5xl font-black italic font-serif text-zinc-900 tracking-tight mb-2">Word Library</h1>
                  <p className="text-zinc-500 font-medium tracking-tight">
                    Study and master over <span className="text-[#58007E] font-black">500+</span> essential English words in 8 key categories.
                  </p>
                </div>
                {mappedLearnedDetailed.length > 0 && (
                  ...
                  >
                    <Brain size={18} /> Practice {mappedLearnedDetailed.length} Flashcards
                  </button>
                )}
              </header>
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                <div className="bg-white p-6 rounded-[32px] border border-zinc-100 shadow-sm text-center flex flex-col justify-center">
                  <p className="text-[10px] font-black uppercase text-zinc-400 tracking-widest mb-1">Total Library</p>
                  <p className="text-3xl font-black text-zinc-950">{allWords.length} words</p>
                </div>
                <div className="bg-white p-6 rounded-[32px] border border-zinc-100 shadow-sm text-center flex flex-col justify-center">
                  <p className="text-[10px] font-black uppercase text-zinc-400 tracking-widest mb-1">Learned Words</p>
                  <p className="text-3xl font-black text-emerald-500">{mappedLearnedDetailed.length} learned</p>
                </div>
                <div className="bg-white p-6 rounded-[32px] border border-zinc-100 shadow-sm text-center flex flex-col justify-center">
                  <p className="text-[10px] font-black uppercase text-zinc-400 tracking-widest mb-1">Active Study Level</p>
                  <p className="text-2xl font-black text-[#58007E] uppercase">{profile?.level || 'A1'}</p>
                </div>
                <div className="bg-white p-6 rounded-[32px] border border-zinc-100 shadow-sm text-center flex flex-col justify-center">
                  <p className="text-[10px] font-black uppercase text-zinc-400 tracking-widest mb-1">XP Multiplier</p>
                  <p className="text-3xl font-black text-amber-500">{profile?.xp || 450} XP</p>
                </div>
              </div>
              ...
              <div className="bg-white border border-zinc-100 rounded-3xl p-6 shadow-sm space-y-4">
                <div className="relative">
                  ...
                  <input 
                    ...
                    className="w-full bg-zinc-50 border border-transparent focus:border-zinc-100 focus:bg-white rounded-[24px] py-4.5 pl-16 pr-8 text-sm font-bold outline-none transition-all"
                  />
                </div>

                <div className="grid grid-cols-1 md:grid-cols-3 gap-4 pt-1">
                  <div>
                    <label className="text-[10px] font-black uppercase text-zinc-400 tracking-widest mb-1.5 block">Category Filter</label>
                    <select
                  ...
                  <div>
                    <label className="text-[10px] font-black uppercase text-zinc-400 tracking-widest mb-1.5 block">Level (CEFR)</label>
                    <select
                      value={selectedLevel}
                  ...
                  <div>
                    <label className="text-[10px] font-black uppercase text-zinc-400 tracking-widest mb-1.5 block">Part of Speech</label>
                    <select
                      value={selectedPos}
                  ...
                  {filteredWords.map(word => {
                    ...
                    return (
                      <motion.div 
                        ...
                        className="bg-white p-8 rounded-[40px] border border-zinc-100 shadow-sm hover:shadow-xl transition-all group relative overflow-hidden flex flex-col justify-between min-h-[340px]"
                      >
                        <div>
                          {/* Title / Badges */}
                          <div className="flex justify-between items-start mb-4">
                            <div>
                              ...
                              <h3 className="text-2xl font-black italic font-serif text-zinc-900 group-hover:text-[#58007E] transition-colors inline-block mr-2">{word.word}</h3>
                              <button 
                            ...
                            >
                              {isLearned ? <Check size={12} strokeWidth={3} /> : <CheckCircle2 size={12} />}
                          ...
                          <p className="text-sm font-bold text-zinc-400 tracking-tight mb-4 lowercase">
                            Монгол : <strong className="text-zinc-800 font-extrabold">{word.translation}</strong>
                          </p>

                          {/* English Definition details */}
                          <div className="text-[10px] font-black uppercase tracking-wider text-zinc-400 mb-1">Category</div>
                          <p className="text-xs font-semibold text-zinc-600 mb-5">{word.category}</p>

                          {/* Dual Example block */}
                          <div className="bg-zinc-50 p-5 rounded-3xl border border-zinc-100 space-y-2">
                            <div>
                              <p className="text-[8px] font-black uppercase tracking-widest text-zinc-400">English Context</p>
                              <p className="text-xs font-bold text-zinc-800 italic">"{word.example}"</p>
                            </div>
                            <div className="border-t border-zinc-100 pt-2">
                              <p className="text-[8px] font-black uppercase tracking-widest text-[#58007E]/55">Mongolian Context</p>
                              <p className="text-xs font-semibold text-zinc-500 italic">"{word.exampleMn}"</p>
                            </div>
                          </div>
                        </div>
                        ...
                        {isLearned && (
                          <div className="text-[8px] font-bold text-emerald-500 uppercase tracking-widest mt-4 text-right flex items-center justify-end gap-1">
                            <Clock size={10} /> Learned {word.dateLearned ? new Date(word.dateLearned).toLocaleDateString() : 'recently'}
                          </div>
                        )}
                      </motion.div>
                    );
              ...
              {filteredWords.length === 0 && (
                <div className="py-24 text-center bg-white rounded-[40px] border border-dashed border-zinc-200 shadow-sm max-w-xl mx-auto">
                  <div className="w-16 h-16 bg-zinc-50 rounded-full flex items-center justify-center mx-auto mb-4 text-zinc-300">
                    <BookOpen size={28} />
                  </div>
                  <p className="text-zinc-500 font-bold text-xs uppercase tracking-widest">No matching words found.</p>
                  <p className="text-zinc-400 text-xs mt-1">Try to broad your search terms or verify level filter.</p>
                </div>
              )}
            </motion.div>
          ) : (
            ...
            >
              {!practiceFinished ? (
                <div className="space-y-8">
                  ...
                  <div className="flex justify-between items-center bg-white border border-zinc-100 rounded-3xl px-6 py-4 shadow-sm">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <h3 className="text-base font-black italic font-serif">Vocabulary Practice</h3>
                        <p className="text-[9px] font-bold text-zinc-400 uppercase tracking-wider mt-0.5">Active Session</p>
                      </div>
                    </div>
                    <button 
                  ...
                  <div className="flex justify-between text-[10px] font-black uppercase tracking-widest text-zinc-400">
                    <span>Card {currentFlashcardIdx + 1} of {practicePool.length}</span>
                    <span className="text-emerald-500">Known: {sessionScore.correct}</span>
                  </div>
                  ...
                  <div className="w-full h-2 bg-zinc-100 rounded-full overflow-hidden">
                    <motion.div 
                      ...
                      initial={{ width: 0 }}
                      animate={{ width: `${((currentFlashcardIdx + 1) / practicePool.length) * 100}%` }}
                    />
                  </div>

                  {/* The Flapping 3D Card Base container */}
                  <div 
                    className="relative cursor-pointer min-h-[380px] perspective-1000 group w-full"
                  ...
                  >
                    <motion.div
                      ...
                      className="w-full h-full min-h-[380px] relative bg-white border border-zinc-200/60 rounded-[48px] shadow-2xl p-10 flex flex-col justify-between"
                    >
                      {/* FRONT FACE (English) */}
                      <div 
                      ...
                      >
                        <div className="flex justify-between items-start">
                          <span className="px-3 py-1 bg-[#58007E]/5 text-[#58007E] text-[9px] font-black rounded uppercase tracking-wider">
                            {practicePool[currentFlashcardIdx].level} • {practicePool[currentFlashcardIdx].pos}
                          </span>
                          <span className="text-[9px] font-bold text-zinc-300 uppercase tracking-widest">Click to flip</span>
                        </div>

                        <div className="text-center my-auto">
                          <h2 className="text-5xl font-black italic font-serif text-zinc-900 tracking-tight leading-none mb-4">{practicePool[currentFlashcardIdx].word}</h2>
                          <p className="text-zinc-400 font-bold text-xs uppercase tracking-widest">How do you translate this term?</p>
                        </div>

                        <div className="flex gap-4 items-center justify-center text-xs font-semibold text-zinc-400">
                          <Brain size={16} /> Category: {practicePool[currentFlashcardIdx].category}
                        </div>
                      </div>
                      ...
                      <div 
                        ...
                        className="absolute inset-0 p-10 flex flex-col justify-between bg-zinc-950 rounded-[48px] text-white"
                      >
                        <div className="flex justify-between items-start">
                          ...
                          <span className="text-[9px] font-bold text-zinc-500 uppercase tracking-widest">Click to flip</span>
                        </div>

                        <div className="text-center my-auto px-4">
                          <h4 className="text-[10px] font-black uppercase text-zinc-500 tracking-wider mb-2">Mongolian meaning</h4>
                          <h2 className="text-4xl font-extrabold italic text-[#AB4DFF] mb-6">{practicePool[currentFlashcardIdx].translation}</h2>
                          
                          <div className="text-left space-y-3 bg-white/5 p-5 rounded-3xl border border-white/5">
                            <div>
                              <p className="text-[8px] font-black text-zinc-400 uppercase tracking-wider">En Example</p>
                              <p className="text-xs font-bold text-zinc-200 italic">"{practicePool[currentFlashcardIdx].example}"</p>
                            </div>
                            <div className="border-t border-white/5 pt-2">
                              <p className="text-[8px] font-black text-indigo-400 uppercase tracking-wider">Mn Translation</p>
                              <p className="text-xs font-medium text-zinc-300 italic">"{practicePool[currentFlashcardIdx].exampleMn}"</p>
                            </div>
                          </div>
                        </div>
                ...
                <motion.div 
                  initial={{ opacity: 0, scale: 0.95 }}
                  animate={{ opacity: 1, scale: 1 }}
                  className="bg-white border border-zinc-100 rounded-[48px] p-12 text-center shadow-2xl relative overflow-hidden"
                >
                  <div className="w-24 h-24 bg-emerald-50 text-emerald-500 rounded-[36px] flex items-center justify-center mx-auto mb-8 shadow-inner animate-bounce">
                    <Trophy size={48} />
                  </div>

                  <h2 className="text-4xl font-black italic font-serif leading-none tracking-tight mb-2">Practice Completed!</h2>
                  <p className="text-zinc-500 font-medium mb-8 max-w-sm mx-auto">Great job mastering your customized learned words array!</p>

                  <div className="bg-zinc-50 px-10 py-8 rounded-[36px] inline-block mb-10 border border-zinc-100 text-center min-w-[240px]">
                    <div className="text-5xl font-black text-[#58007E]">{sessionScore.correct} / {practicePool.length}</div>
                    <div className="text-[9px] font-black text-zinc-400 uppercase tracking-[0.2em] mt-2">Active Score Accuracy</div>
                  </div>
                  ...
                  <div className="max-w-md mx-auto mb-8 text-left bg-emerald-50/30 p-6 rounded-3xl border border-emerald-50/60 flex items-start gap-4">
                    ...
                    <div>
                      <p className="text-xs font-black uppercase text-emerald-600 tracking-wider mb-1">AI Coaching insight</p>
