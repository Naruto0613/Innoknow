...
export interface UserProfile {
  ...
  xp: number;
  streak: number;
  learnedWords?: { wordId: string; dateLearned: string }[];
...
export const AuthProvider: React.FC<{ children: React.ReactNode }> = ({
  ...
  useEffect(() => {
    const initSession = async () => {
      ...
      const allUsers = JSON.parse(
        localStorage.getItem("innoknow_registered_users") || "{}",
      );
      ...
      if (storedUser) {
        try {
          ...
          try {
            ...
            console.warn(
              "Could not fetch user profile from Firestore, falling back to LocalStorage:",
              e,
            );
          }
          ...
          if (fetchedProfile) {
            ...
            try {
              ...
              console.warn(
                "Firestore collection setDoc failed, offline fallback preserved.",
              );
            }
          }
        } catch (err) {
  ...
  const login = async (email: string, password: string) => {
    ...
    try {
      ...
      console.warn(
        "Firestore fetch failed on login, fallback to local database.",
      );
    }
  ...
  const signup = async (
    ...
    const usedTrials = JSON.parse(
      localStorage.getItem("innoknow_used_trials") || "{}",
    );
    const usedTrialDevices = JSON.parse(
      localStorage.getItem("innoknow_used_trial_devices") || "{}",
    );
    ...
    const trialExpiryVal = hasEligibleTrial
    ...
    if (hasEligibleTrial) {
      usedTrials[normalizedEmail] = {
        email: normalizedEmail,
        dateUsed: now.toISOString(),
      };
      usedTrialDevices[currentDeviceID] = {
        deviceId: currentDeviceID,
        email: normalizedEmail,
        dateUsed: now.toISOString(),
      };
      ...
      localStorage.setItem(
        "innoknow_used_trial_devices",
        JSON.stringify(usedTrialDevices),
      );
    }
