...
export const getTutorResponse = async (level: string, message: string, history: any[] = []) => {
...
export const getFeedback = async (level: string, text: string) => {
  ...
  try {
    ...
    return { score: 70, grammar: "Good attempt!", vocabulary: "Keep learning!", suggestions: ["Try to use more varied adjectives."] };
  }
};
