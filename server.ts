...
async function startServer() {
  ...
  app.post("/api/ai/generate", async (req, res) => {
    ...
    try {
      ...
      console.warn("Gemini API Error (using smart local fallback):", error.message || error);
      
      // Smart Rule-based local feedback generator if AI key fails
  ...
  app.post("/api/ai/writing", async (req, res) => {
    try {
      ...
      const response = await ai.models.generateContent({
        ...
        config: {
          systemInstruction: "You are an English teacher for Mongolian students. Give feedback on: 1. Grammar mistakes (list them), 2. Vocabulary suggestions, 3. Structure, 4. What they did well, 5. Overall score out of 10. Keep feedback encouraging and simple. They are a Mongolian learner so be patient and kind.",
        },
      });

      res.json({ feedback: response.text });
    } catch (error: any) {
      console.warn("Gemini API Error (using smart writing fallback):", error.message || error);
      
      const wordCount = (req.body.text || "").trim().split(/\s+/).filter(Boolean).length;
      const suggestions = [
  ...
  app.post("/api/ai/speaking", async (req, res) => {
    try {
      ...
      const response = await ai.models.generateContent({
        ...
        config: {
          systemInstruction: "You are an English speaking coach. The user just practiced a speaking topic. Here is their transcript. Provide brief feedback on their grammar, suggest more natural ways to say things, and give them a quick encouragement score out of 10.",
        },
      });

      res.json({ feedback: response.text });
    } catch (error: any) {
      console.warn("Gemini API Error (using smart speaking fallback):", error.message || error);
